...
def client_with_mocked_services(
    ...
    app.dependency_overrides[get_settings] = lambda: integration_settings
    app.dependency_overrides[get_ingestion_state_service] = lambda: mock_state_service
    app.dependency_overrides[get_file_management_service] = lambda: mock_file_service
    app.dependency_overrides[get_collection_manager_service] = lambda: mock_collection_service
    app.dependency_overrides[get_ingestion_processor_service] = lambda: mock_ingestion_processor
...
def test_pdf_file(integration_settings, sample_pdf_content, tmp_path):
    ...
    # Create a simple text file instead of PDF to avoid reportlab dependency
    pdf_path = Path(integration_settings.SOURCE_DIRECTORY) / "test_document.pdf"

    # Create a simple text file that pretends to be a PDF
