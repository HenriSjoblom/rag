...
class TestProcessMethod:
    ...
    async def test_process_success(self, chat_processor_service, mocker):
        ...
        result = await chat_processor_service.process("test query")
    ...
    async def test_process_retrieval_failure(self, chat_processor_service, mocker):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await chat_processor_service.process("test query")
    ...
    @pytest.mark.asyncio
    async def test_process_logs_query_info(
        self, chat_processor_service, mocker, caplog
    ):
        """Test that process method logs query information."""
        ...
        # Check that query is logged (query is short so no truncation)
        assert "Processing chat query: 'test query...'" in caplog.text
        assert "Generated AI response" in caplog.text
